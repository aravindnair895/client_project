...
def save_staff(request):
    ...
    if request.method == "POST":
        ...
        email = request.POST.get("email")
        address = request.POST.get("address")
        m_salary = request.POST.get("monthly_salary") or None
        d_wage = request.POST.get("daily_wage")or None
...
def update_staff(request, stf_id):
    ...
    if request.method == "POST":
        ...
        tab_stf.monthly_salary = m_salary
        tab_stf.daily_wage = d_wage
        tab_stf.save()
...
def salary_status(request):
    ...
    # -------------------------------
    # If user selects month manually
    # -------------------------------
    if selected_date:
        selected_dt = datetime.strptime(selected_date, "%Y-%m")
    ...
    if month and year:
        ...
        selected_month_value = date(year, month, 1).strftime("%Y-%m")
    else:
    ...
    return render(
        ...
        "salary_status.html",
        {
            "sal": page_obj,
            "page_obj": page_obj,
            "selected_month": selected_month_display,
...
def salary_generate(request):
    staff = TableStaffs.objects.filter(status__iexact="active")
    return render(request, "salary_generate.html", {"staff": staff})


@login_required(login_url='/')
# def generate_msalary(request):
#
#     if request.method == "POST":
#
...
#             else:
#
#                 days = int(request.POST.get(f"days_{staff.id}", 0))
#                 base_salary = (staff.daily_wage or 0) * days
#
#             TableSalary.objects.create(
#                 staff=staff,
#                 date=date(year, month, salary_date.day),
...
def generate_msalary(request):
    ...
    if request.method == "POST":
        ...
        staffs = TableStaffs.objects.filter(staff_type="permanent",status__iexact="active")
...
def generate_tsalary(request):
    ...
    if request.method == "POST":
        ...
        selected_staff_ids = request.POST.getlist("selected_staffs")
        ...
        for staff_id in selected_staff_ids:
            ...
            staff = TableStaffs.objects.get(id=staff_id)

            # Prevent duplicate weekly salary
            if TableSalary.objects.filter(
                staff=staff,
                date=salary_date
            ).exists():
                ...
                messages.warning(
                    request,
                    f"Salary already generated for {staff.name} this week."
                )
                skipped_count += 1
                continue
            ...
            # Prevent zero-day salary
            if days <= 0:
                messages.warning(
                    request,
                    f"Enter valid working days for {staff.name}."
                )
                skipped_count += 1
                continue
        ...
        if created_count:
            messages.success(
                request,
                f"{created_count} weekly salary record(s) generated."
            )

        return redirect("/salary_status/")


def delete_staffsalary(request, salary_id):

    salary = TableSalary.objects.get(id=salary_id)

    if salary.status == "paid":
        messages.warning(request, "Cannot delete paid salary.")
        return redirect("/salary_status/")
...
#     response['Content-Disposition'] = f'attachment; filename="salary_slip_{salary.staff.name}.pdf"'
#
...
def delete_salary(request):
    ...
    if request.method == "POST":
        ...
        if salaries.filter(status="paid").exists():
            messages.warning(
                request,
                "Cannot delete. Some salaries are already paid."
            )
            return redirect("/salary_status/")
...
def salary_history(request):
    ...
    search_stafftype = request.GET.get("search_stafftype")
    ...
    if search_stafftype:
        data = data.filter(staff__staff_type=search_stafftype)
